...
from models.jax_td3 import TD3Config, make_td3_states, td3_critic_update, td3_actor_update, soft_update
...
def evaluate(env, actor_params, actor_apply, n_actions, crop_size, episodes, step, run_dir: Path):
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            a = actor_apply({'params': actor_params}, jnp.asarray(x))
            a = np.asarray(a[0])
            act = int(np.argmax(a))
...
class Replay:
    def __init__(self, capacity, obs_shape, n_actions):
        ...
        self.act = np.zeros((self.N, n_actions), np.float32)  # [-1,1]
        self.rew = np.zeros((self.N,), np.float32)
        self.done= np.zeros((self.N,), np.float32)
        self.ptr=0; self.size=0
    def add(self, o,a,r,n,d):
        self.obs[self.ptr]=o; self.act[self.ptr]=a; self.rew[self.ptr]=r
        self.next[self.ptr]=n; self.done[self.ptr]=d
        self.ptr=(self.ptr+1)%self.N; self.size=min(self.size+1,self.N)
...
def train_td3(env_name: str,
    ...
    cfg = TD3Config(batch_size=batch_size, seed=seed)
    rng = jax.random.PRNGKey(seed)
    (rng, actor_state, c1_state, c2_state,
     t_actor_params, t_q1_params, t_q2_params) = make_td3_states(rng, obs_shape, n_actions, cfg)
    ...
    while global_steps < total_steps:
        # Exploration noise in action space (actor outputs [-1,1], add small noise)
        a = actor_state.apply_fn({'params': actor_state.params}, jnp.asarray(cur[None, ...]))
        a = np.asarray(a[0])
        noise = np.clip(np.random.normal(0, 0.1, size=a.shape), -0.3, 0.3)
        a_expl = np.clip(a + noise, -1.0, 1.0)

        # Map to discrete action for env
        a_disc = int(np.argmax(a_expl))
        next_obs, r, term, trunc, info = env.step(a_disc)
        done = term or trunc
        nxt = obs_to_cropped_onehot(next_obs, n_actions, crop_size)

        replay.add(cur, a_expl, float(r), nxt, float(done))
        ...
        if replay.size >= max(batch_size, warmup):
            ...
            rng, c1_state, c2_state, cmetrics = td3_critic_update(
                actor_state, c1_state, c2_state,
                t_actor_params, t_q1_params, t_q2_params,
                batch, cfg, rng
            )

            # Delayed actor update
            if (global_steps % cfg.policy_delay) == 0:
                actor_state, ametrics = td3_actor_update(actor_state, c1_state, batch)
                # Soft update targets
                t_actor_params = soft_update(actor_state.params, t_actor_params, cfg.tau)
                t_q1_params    = soft_update(c1_state.params, t_q1_params, cfg.tau)
                t_q2_params    = soft_update(c2_state.params, t_q2_params, cfg.tau)

        # Eval + ckpt
        if (global_steps % eval_every == 0) or (global_steps >= total_steps):
            res = evaluate(env, actor_state.params, actor_state.apply_fn, n_actions, crop_size, 10, global_steps, run_dir)
            print(f"[TD3] step={global_steps} return={res['return_mean']:.3f} quality={res['quality_mean']:.3f}")
        ...
        if (global_steps % save_every == 0) or (global_steps >= total_steps):
            checkpoints.save_checkpoint(run_dir / "checkpoints", target={
                ...
                'critic1': c1_state.params,
                'critic2': c2_state.params,
                't_actor': t_actor_params,
                't_q1': t_q1_params,
                't_q2': t_q2_params
            }, step=global_steps, overwrite=True, keep=3)
            print(f"[TD3] saved checkpoint @ step {global_steps}")
...
def main():
    ...
    train_td3(args.env, Path(args.run_dir),
