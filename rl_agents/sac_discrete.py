...
class SACDiscreteAgent(RLAgentBase):
    ...
    def _update(self, s, a, r, s2, d):
        ...
        with torch.no_grad():
            ...
            alpha = self.log_alpha.exp() if hasattr(self, "log_alpha") else torch.tensor(self.alpha, device=self.device)
        ...
        pi = torch.softmax(logits, dim=-1); log_pi = torch.log_softmax(logits, dim=-1)
        with torch.no_grad():
        ...
        return {"loss/q": q_loss.detach(), "loss/pi": pi_loss.detach(), "alpha": (self.log_alpha.exp().detach() if hasattr(self, "log_alpha") else torch.tensor(self.alpha))}
